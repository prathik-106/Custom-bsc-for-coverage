...
Usage:
    ...
    # --verilog-dir also accepts a single .v file directly, instead of a
    # directory to glob -- useful for quick single-module checks where
    # scanning a whole build dir is unnecessary overhead:
    python3 coverage_report.py --verilog-dir build/hw/verilog/mkExample.v \
...
def main():
    ...
    ap.add_argument('--verilog-dir', required=True,
                     help='Directory containing generated .v files '
                          '(e.g. build/hw/verilog), OR a path to a '
                          'single .v file to scan directly.')
    ...
    # --verilog-dir may be either a directory (glob *.v inside it, the
    # original behavior) or a direct path to a single .v file -- avoids
    # forcing a full-directory scan (and needing --file to narrow it
    # back down afterward) when you only care about one module.
    if os.path.isfile(args.verilog_dir):
        v_files = [args.verilog_dir]
    else:
        v_files = glob.glob(f'{args.verilog_dir}/*.v')
    if not v_files:
        print(f'ERROR: no .v files found at {args.verilog_dir}', file=sys.stderr)
        sys.exit(1)
